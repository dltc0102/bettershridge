...
register('gameLoad', () => {
    ChatLib.chat(`&9[&bBetterShridge&9] &3Loaded!`)
    ...
    ChatLib.chat(`&7 >> If any problems found, dm obiscuit!`)
});
...
registerWhen('chat', timeThis('guild message time', (playerInfo, playerRole, playerStuff, event) => {
    ...
    if (data.bots.includes(playerName)) {
        ...
        //! demoted from
            ...
            if (guildDebug) ChatLib.chat('promoted from -- response');
            let demotedMessage = getGuildResponse(BOT_PREFIX, playerMessage, 'promoted')
